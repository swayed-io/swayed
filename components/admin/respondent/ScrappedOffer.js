...
function ScrappedOffer({ offer }) {
  ...
  function convertToUSDate(timestamp) {
    const date = new Date(timestamp);
    const month = date.getUTCMonth() + 1; // Get the month (0-11) and add 1 to get the correct month number
    const day = date.getUTCDate(); // Get the day (1-31)
    const year = date.getUTCFullYear().toString().substr(-2); // Get the year and take the last 2 digits

    return `${month}/${day}/${year}`;
  }

  function convertToUSDate2(timestamp) {
    if (timestamp === null) {
      return "Null";
    }
    const date = new Date(timestamp);
    ...
    const year = date.getFullYear().toString().substr(-2); // Get the year and take the last 2 digits
      ...
      <div className="flex flex-col sm:p-4 p-2">
        ...
        <div className="flex flex-row space-x-2  ">
          ...
          <button className="rounded-full bg-background-100 text-center py-2 px-4 font-medium text-sm   flex flex-row gap-2 ">
            ...
            />
            <span className="opacity-60">{offer?.facility}</span>
          </button>
          <button className="rounded-full bg-background-100 text-center py-2 px-4 font-medium text-sm   flex flex-row gap-2 ">
            <img
          ...
          <div className="flex sm:flex-row flex-col space-x-2">
            <div>
              ...
              <p className="mt-2 text-sm text-black font-medium text-opacity-75">
                {offer?.gender} with {offer?.age} years
              </p>
            </div>
            <div>
              <p className="font-medium text-black text-opacity-60 text-sm">
              ...
              <p className="mt-2 text-sm text-black font-medium text-opacity-75">
                {convertToUSDate(offer?.published)} -{" "}
                {convertToUSDate2(offer?.expires)}
              </p>
            </div>
          </div>
          <div className="">
            <button className="px-4 py-2 font-medium bg-gradient-to-tr from-primary-100 to-secondary-100 text-white rounded-full border-0  focus:outline-none hover:bg-gray-200  sm:ml-0 ml-64  text-sm mt-8 transition ease-in-out delay-150 bg-blue-500 hover:-translate-y-1 hover:scale-110">
              <Link href={offer?.url}>View Project</Link>
            </button>
          </div>
        </div>
      </div>
